...
// IssueGetCommandByNameWithObject issues the specified read command and the settings supports object value type
func (client *CommandClient) IssueGetCommandByNameWithObject(ctx context.Context, deviceName string, commandName string, dsPushEvent string, dsReturnEvent string, settings map[string]interface{}) (res *responses.EventResponse, err errors.EdgeX) {
	requestParams := url.Values{}
	requestParams.Set(common.PushEvent, dsPushEvent)
	requestParams.Set(common.ReturnEvent, dsReturnEvent)
	requestPath := path.Join(common.ApiDeviceRoute, common.Name, url.QueryEscape(deviceName), url.QueryEscape(commandName))
	err = utils.GetRequestWithRawData(ctx, &res, client.baseUrl, requestPath, requestParams, settings)
	if err != nil {
		return res, errors.NewCommonEdgeXWrapper(err)
	}
	return res, nil
