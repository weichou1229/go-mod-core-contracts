...
type DeviceServiceCommandClient interface {
	...
	// GetCommandWithObject invokes device service's get command API and the settings supports object value type
	GetCommandWithObject(ctx context.Context, baseUrl string, deviceName string, commandName string, queryParams string, settings map[string]interface{}) (*responses.EventResponse, errors.EdgeX)
